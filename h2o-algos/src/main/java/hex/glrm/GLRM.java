 ...
 */
      ...
      try {
        ...
        while (!isDone(model, steps_in_row, step)) {
          ...
          if (_wideDataset) {
            ...
            yextsk.doAll(dinfo._adaptedFrame);
          } else {
  ...
  //--------------------------------------------------------------------------------------------------------------------
  // Update Y equivalent to X step for wide datasets.
  //--------------------------------------------------------------------------------------------------------------------
  ...
  private static class UpdateYeX extends MRTask<UpdateYeX> {
    ...
    UpdateYeX(GLRMParameters parms, Archetypes yt, double alpha, boolean update, int ncolA, int ncolX, int ncats,
            double[] normSub, double[] normMul, GlrmLoss[] lossFunc, int weightId) {
      assert yt != null && yt.rank() == ncolX;
      _parms = parms;
      _yt = yt;
...
/*  // under wide dataset, this is equivalent to updating Y, doing it column at a time
  ...
  private static class UpdateY extends MRTask<UpdateY> {
    ...
    @Override protected void postGlobal() {
      ...
      for (int j = 0; j < _ytnew.length; j++) {
		...
		private static class ObjCalcW extends MRTask<ObjCalcW> {
				...
				// Output
				double _loss;       // Loss evaluated on A - XY using new X (and current Y)
				static double _xold_reg;   // Regularization evaluated on old X st
      ...
      if ((_regX) && (startxcidx == 0)) {
        ...
        ArrayList<Integer> xChunkIndices = findXChunkIndices(_xVecs, tArowStart, tArowEnd, _yt);  // contains x chunks to get
      ...
      for (int j = 0; j < xWidth; ++j) {  // read in the relevant xVec chunks
        xChunks[j] = xVecs.vec(j).chunkForChunkIdx(chunkIdx);
				...
				private ArrayList<Integer> findXChunkIndices(Frame xVecs, int taStart, int taEnd, Archetypes yt) {
      ...
      taStart = findExpColIndex(taStart, numCats, yt);  // translate col indices to expanded column index with categoricals
    ...
    public void findGoodCidx(Frame xVecs, ArrayList<Integer> currentList, boolean addIndex, int taIndex, int xNChunks, int startTAcidx) {
